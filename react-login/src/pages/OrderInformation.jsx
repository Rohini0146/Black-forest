...
const OrderInformation = () => {
  ...
  useEffect(() => {
    if (orders.length > 0) {
      filterOrders(activeTab); // Apply filter on initial load or when tab changes
    }
  }, [orders, activeTab]);
  ...
  // Fetch orders within the specified date range based on filter type
  const fetchFilteredOrders = async (startDate, endDate, dateType) => {
    setLoading(true);
    try {
      const response = await axios.get(
        `http://43.205.54.210:3001/orders?startDate=${startDate}&endDate=${endDate}&dateType=${dateType}`
      );
  ...
  // Handle date range change based on the selected filter type (Order Date or Delivery Date)
  const handleRangeChange = (dates) => {
    if (dates) {
      ...
      fetchFilteredOrders(
        ...
        filterType // Pass the selected filter type
      );
    } else {
