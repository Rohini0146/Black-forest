...
import { Card, Select, Button, message, Pagination, DatePicker } from "antd";
...
const ProductView = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Apply filters for branch, delivery date, and order date
  ...
  const groupedProducts = orders.reduce((acc, order) => {
    order.products.forEach((product) => {
      if (!acc[product.name]) {
        acc[product.name] = { totalQuantity: 0, branches: {} };
      }

      acc[product.name].totalQuantity += product.quantity;
      if (!acc[product.name].branches[order.branch]) {
        acc[product.name].branches[order.branch] = 0;
      }
      acc[product.name].branches[order.branch] += product.quantity;
    });
          ...
          <Card key={productName} className="order-card">
            <div className="order-details-1">
              ...
              <div className="order-action">
                <p style={{ fontSize: "22px", whiteSpace: "nowrap" }}>
                  <b>Qty: </b> {product.totalQuantity} Pcs
                </p>
                <Button
                  type="primary"
